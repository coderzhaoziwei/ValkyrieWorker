(function () {
  ...
  var LoginMethods = {
    ...
    ToRegist: function () {
      ...
      API.UserAPI.IsRegistValidation(function (x) {
        LoginMethods.isRegistValidation = x;
        if (x) {
          $('#regist_valpanel').show();
          LoginMethods.GetValidationImage();
        } else {
          $('#regist_valpanel').hide();
        }
      });
    }, GetValidationImage: function () {
      API.UserAPI.ValidationImage(function (x) {
        $(".validnum-box>.validnum-img").attr('src', "data:image/jpeg;base64," + x);
      });
    },
    ToUpdate: function () {
    ...
    ResetPwd: function () {
      ...
      if (!/^1\d{10}$/.test(phone)) return ShowInputError("#reset_phone", "手机号码格式错误");
      ...
      if (!phone.is(":disabled")) {
        ...
        if (!/^1\d{10}$/.test(phone_no)) return ShowInputError("#reset_phone", "手机号码格式错误");
      }
      ...
      if (!pwd2) return ShowInputError("#phone_pwd", "请重复输入你的新密码");
      if (pwd2.length < 6 || pwd2.length > 20) return ShowInputError("#phone_pwd", "密码长度在6到20之间");


      API.UserAPI.BindPhone(valid_no, phone_no, pwd2, function (x) {
        if (x) {
    ...
    DeleteRole: function () {
      ...
      var valno = null;
      if (LoginMethods.isRegistValidation) {
        valno = $("#regist_val").val();
      ...
      var user = {
        Name: name, PassWord: pwd, ValidationNo: valno
      };
      API.UserAPI.Regist(user, function (x) {
        if (x.code == 1) {
          ShowServers();
        } else if (x.code == -1) {
          ShowInputError("#regist_valpanel", x.message || '验证码错误');
          HideAndShow("#regist_panel");
        } else {
  ...
  function SendValidateCode() {
    ...
    if (!txt.is(":disabled")) {
      ...
      if (!/^1\d{10}$/.test(phone)) return ShowInputError(txt, "手机号码格式错误");
    } else {
      phone = "";
    }
  ...
  function DisplayServer() {
    ...
    for (var i = 0; i < SERVERS.length; i++) {
      if (SERVERS[i].IsTest && window.location.search != "?test" && window.location.hostname != "localhost"
        && window.location.host !== 'test.wsmud.com') {
        SERVERS.splice(i, 1);
        continue;
      }
  ...
  Dialog.list = {
    ...
    updateitem: function (data) {
      ...
      if (this.isstore) {
        ...
        Dialog.title(!this.is_bookshelf ? "你的仓库中有" + this.stores.length + "/" + this.max_store_count + "件物品" :
          "你的书架上有" + this.stores.length + "/" + this.max_store_count + "本秘籍");
      }
      this.update_pack();
      ...
      var gongji = data.gongji || data.jungong || data.yaoyuan;
      if (data.selllist) {
        this.isstore = false;
        this.gongji = gongji;
        ...
        else if (data.yaoyuan) this.money_name = "<ord>妖元</ord>";
      ...
      if (data.stores) {
        ...
        this.is_bookshelf = !!data.bookshelf;
        Dialog.titleElement.html(!this.is_bookshelf ? "你的仓库中有" + data.stores.length + "/" + data.max_store_count + "件物品"
          : "你的书架上有" + data.stores.length + "/" + data.max_store_count + "本秘籍");
    ...
    , item_click: function () {
      ...
      if (Dialog.list.isstore) {
        if (otype == 3) {
          html.push('<span cmd="checkobj ' + obj + ' from ' + (Dialog.list.is_bookshelf ? "sj" : "store") + '">查看</span>');
          html.push('<span cmd="_confirm qu ' + obj + '">取出</span>');
        } else if (otype == 1) {
          html.push('<span cmd="checkobj ' + obj + ' from item">查看</span>');
          if (Dialog.list.is_bookshelf)
            html.push('<span cmd="_confirm store ' + item.count + ' ' + obj + '">放到书架</span>');
          else
  ...
  Dialog.shop = {
    close: function () {
      ...
      if (!this.isShow) return;
      if (window.isios) {
        Dialog.footer("<div class='dialog-shop-footer'>你身上有<hij> " + this.cash_money + " 元宝</hij><span cmd='transmoney'>详情</span></div>");
      } else {
        Dialog.footer("<div class='dialog-shop-footer'>你身上有<hij> " + this.cash_money + " 元宝</hij><span cmd='transmoney'>详情</span><span cmd='pay'>充值</span><span cmd='tg'>推广</span></div>");
      }
  ...
  API.UserAPI = {
    ...
    ValidationImage: function (cb) {
      return Util.Get('UserAPI/ValidationImage', cb);
    },
    ...
    BindPhone: function (code, no, pwd, cb) {
      return Util.Post('UserAPI/BindPhone', { code: code, no: no, pwd: pwd }, cb);
    },
